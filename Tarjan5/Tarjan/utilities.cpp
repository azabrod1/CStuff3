...
Graph<Vid>* Utility::importGraphFromCSP(std::string filename){
    ...
    std::unordered_map<Vid, std::unordered_set<Vid>* >* graph = new std::unordered_map<Vid, std::unordered_set<Vid>*>;
    ...
    while(std::getline(file, line)){
        ...
        std::unordered_set<Vid>* neighbors(new std::unordered_set<Vid>);
    ...
    return new DirectedHashGraph<Vid>(std::move(*graph));
}
