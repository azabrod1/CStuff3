...
Worker::Worker(unsigned int _ID, MultiThreadedTarjan& _algo, const Graph<Vid>& _graph, Dictionary<Vid, Cell<Vid>*>& _dict) : ID(_ID), MASK(1<<_ID), scheduler(_algo), graph(_graph), dict(_dict), spareSearch(new Search) {
    
    allocateSpareCell();
    recycledCells.reserve(1000);
}
...
void Worker::initNeighbors(Cell<Vid>* cell){
   ...
   // initNeighbors2(cell);
    ...
    for(auto& succ: succs){
    //    if(spareCell->vertex == succ) allocateSpareCell();;
        ...
        neighbor = status.first;
        cellAge = neighbor->age.load();
        
        if(neighbor->vertex == succ && !neighbor->isComplete() && cellAge == neighbor->age)
            cell->addNeighbor(neighbor, cellAge);
