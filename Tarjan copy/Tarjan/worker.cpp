...
Worker::Worker(unsigned int _ID, MultiThreadedTarjan& _algo, const Graph<Vid>& _graph, Dictionary<Vid, WeakReference<Cell<Vid>>>& _dict) : ID(_ID), MASK(1<<_ID), scheduler(_algo), graph(_graph), dict(_dict), spareSearch(new Search) {
...
void Worker::execute(Search* search){
    Cell<Vid>* curr, *child;
    char attempt;
        ...
        if(!curr->allNeighborsDone()){
            ...
            //case that the cell becomes complete after the conditional but before conquer
                ...
                if(attempt == CellStatus::CLAIMED){
                    ...
                    if(!(search = scheduler.suspend(this, search, child))) return;
