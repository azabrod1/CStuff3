...
Search* SuspensionManager::suspend(Worker& worker, Search* const Sn, Cell<Vid>* const conflictCell){

    const int Sn_Age(Sn->age);
    ...
    if(conflictCell->isComplete()){  //Do we need to test for age??
    ...
    for(int i = 0; i < pathSize; ++i){
        ...
        if(S[i]->tarjanSize() > S[maxIdx]->tarjanSize())
            maxIdx = i;
    ...
    S[maxIdx]->removeCellBlockedOn();

    runCellTransfer(S, C, maxIdx); //Resolve the cycle
...
void SuspensionManager::runCellTransfer(const std::vector<Search*>& S, const std::vector<Cell<Vid>*>& C, const int r)
{
    
    //Traverse the path starting with the search that S is directly blocked on
    for(int i = 0; i < S.size(); ++i){
        if(i == r) continue;
        Search::transferCells(S[i], S[r], C[i]);
    }
    ...
    S[r]->controlStackTop()->promote(C[r]->index);
...
//        //This call may be from a pre-mature resume, make sure we do not accidentally wake another later real suspension
